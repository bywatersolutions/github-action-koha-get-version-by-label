...
if [[ "$VERSION_LABEL" == "master" ]]; then
    ...
    KOHA_VERSION_LINE=$(git ls-remote --heads https://git.koha-community.org/Koha-community/Koha.git | grep '[0-9]\{2\}\.[0-9]\{2\}\.x' | tac | sed '1q;d')
    ...
    KOHA_VERSION_LINE=$(git ls-remote --heads https://git.koha-community.org/Koha-community/Koha.git | grep '[0-9]\{2\}\.[0-9]\{2\}\.x' | tac | sed '2q;d')
    echo "KOHA VERSION LINE: $KOHA_VERSION_LINE"
    IFS='/' read -ra KOHA_VERSION_LINE_PARTS <<< "$KOHA_VERSION_LINE"
    KOHA_VERSION_BRANCH_NAME=${KOHA_VERSION_LINE_PARTS[2]}
fi
